...
import FilterDropdownToCompany from "./ui/filterDropdownToCompany";

export default async function Home() {
  let dataCompanies = await fetch(
  ...
  dataCompanies = await dataCompanies.json();

  const dataRanks = await fetch(
    "https://app-236fde41-0ae0-461e-aa81-9e4a92329356.cleverapps.io/rankers"
  );
  const dataRanksJson = await dataRanks.json();

  // console.log(dataRanksJson);
  ...
  return (
    <div className="min-h-screen h-full">
      <ButtonToAddCompany />
      <FilterDropdownToCompany
        companies={dataCompanies}
        rankers={dataRanksJson}
      />
    </div>
  );
}
